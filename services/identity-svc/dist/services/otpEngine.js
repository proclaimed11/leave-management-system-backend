...
class OtpEngine {
    ...
    users;
    refreshTokens;
    constructor(otpRepo = new otpRepository_1.OtpRepository(), users = new userRepository_1.UserRepository(), refreshTokens = new refreshTokenRepository_1.RefreshTokenRepository()) {
    ...
    async requestOtp(email) {
        ...
        const employee = await (0, directoryClient_1.getDirectoryEmployee)({ email: normalizedEmail });
        ...
        await Promise.all([
            ...
            (0, sendOtpEmail_1.sendOtpEmail)(normalizedEmail, otp),
        ]);
    }
    async verifyOtp(email, otp) {
        ...
        await this.refreshTokens.store(user.id, refreshToken, new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));
        return {
            token,
            refreshToken,
            user: {
